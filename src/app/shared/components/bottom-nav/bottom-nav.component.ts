  ...
  template: `
    <nav class="fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 px-4 py-2 flex justify-around items-center z-50">
      ...
      <a
        ...
        class="flex flex-col items-center p-2 rounded-xl transition-all"
        [class.bg-indigo-100]="isActive('/video-player')"
        [class.dark:bg-indigo-900]="isActive('/video-player')"
        title="Custom Video"
      >
        <svg
          class="w-6 h-6 transition-colors"
          [class.text-indigo-600]="isActive('/video-player')"
          [class.dark:text-indigo-400]="isActive('/video-player')"
          [class.text-slate-500]="!isActive('/video-player')"
          [class.dark:text-slate-400]="!isActive('/video-player')"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
